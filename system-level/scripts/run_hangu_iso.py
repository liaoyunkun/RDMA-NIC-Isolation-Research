...
def execute_program(debug, test_prog, option, params):
    ...
    cmd_list = [
        "cd ../tests/test-progs/hangu-rnic-isolated/src && make",
...
def main():
    ...
    test_prog = "'tests/test-progs/hangu-rnic-isolated/bin/server"
    opt = "'-s " + str(svr_lid) + " -t " + str(num_nodes - 1) + " -m " + str(params.op_mode)
    for i in range(num_nodes - 1):
        test_prog += ";tests/test-progs/hangu-rnic-isolated/bin/client"
